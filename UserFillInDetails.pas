...
type
  TFillInDetailsFormStudent = class(TForm)
    imgQuit: TImage;
    imgBg: TImage;
    imgMathsBtn: TImage;
...
var
  FillInDetailsFormStudent: TFillInDetailsFormStudent;
  ScaleFactor: Double;
...
{$R *.dfm}
procedure TFillInDetailsFormStudent.FormCreate (Sender: TObject);
var
  i: Integer;
begin
  ...
  lblSubmit.Font.Color := RGB(155, 178, 127);
  ...
  dbgrdSelectClassS.Left := imgTableReference.Left + (imgTableReference.Width - dbgrdSelectClassS.Width) div 2;
  dbgrdSelectClassS.Top :=  imgTableReference.Top +45;


    // Calculate the total width of all columns
  TotalWidth := 0;
  for i := 0 to dbgrdSelectClassS.Columns.Count - 1 do
    TotalWidth := TotalWidth + dbgrdSelectClassS.Columns[i].Width;

  // Calculate the width of each column to make them fit the grid's width
  ColumnWidth := dbgrdSelectClassS.Width div dbgrdSelectClassS.Columns.Count;

  // Adjust column widths
  for i := 0 to dbgrdSelectClassS.Columns.Count - 1 do
    dbgrdSelectClassS.Columns[i].Width := ColumnWidth;

  // Adjust the last column width to fit the remaining space
  dbgrdSelectClassS.Columns[dbgrdSelectClassS.Columns.Count - 1].Width :=
  dbgrdSelectClassS.Width - (ColumnWidth * (dbgrdSelectClassS.Columns.Count - 1));
  ...
  lblSubjectSelectLeft := imgbgfromLook.Left + Round(imgbgfromLook.Width * 0.075);
  ...
  lblSelectClassLeft := imgbgfromLook.Left + Round(imgbgfromLook.Width * 0.54);
...
procedure TFillInDetailsFormStudent.AddFontResourceAndBroadcast;
begin
  if AddFontResource('Revision System Project\FONTS\arial-rounded-mt-bold.ttf"') > 0 then
    SendMessage(HWND_BROADCAST, WM_FONTCHANGE, 0, 0);
end;

procedure TFillInDetailsFormStudent.imgQuitClick(Sender: TObject);
begin
...
procedure TFillInDetailsFormStudent.imgMathsBtnClick(Sender: TObject);
begin
 if Maths = True then
 begin
    Maths:= False;
...
procedure TFillInDetailsFormStudent.imgEnglishBtnClick(Sender: TObject);
begin
...
procedure TFillInDetailsFormStudent.imgChemistryBtnClick(Sender: TObject);
begin
...
procedure TFillInDetailsFormStudent.imgComputingBtnClick(Sender: TObject);
begin
...
procedure TFillInDetailsFormStudent.imgBiologyBtnClick(Sender: TObject);
begin
...
procedure TFillInDetailsFormStudent.imgPhysicsBtnClick(Sender: TObject);
begin
...
procedure TFillInDetailsFormStudent.imgGermanBtnClick(Sender: TObject);
begin
    if German = True then
 begin
    German:= False;
...
procedure TFillInDetailsFormStudent.imgSpanishBtnClick(Sender: TObject);
begin
...
procedure TFillInDetailsFormStudent.imgSubmitClick(Sender: TObject);
var
  WhereClause: string;
begin
...
//ensures a user selected a subject
  ...
  if WhereClause <> '' then
    qryFillTable.SQL.Text := qryFillTable.SQL.Text + ' WHERE ' + WhereClause;

  // Set the dataset and open the query
  dsFillTable.DataSet := qryFillTable;
  dbgrdSelectClassS.DataSource := dsFillTable;
  qryFillTable.Open;
end;
end;

procedure TFillInDetailsFormStudent.FindStudentID;
begin
...
procedure TFillInDetailsFormStudent.imgSubmitClassClick(Sender: TObject);
var
  ClassFound: Boolean ;
begin
ClassToAdd := edtClassAdd.Text;

  if SubjectSelected = True then
  begin
  ...
  if not ClassFound then
    ...
    begin
     // Check if a row with the same MainUserID and ClassName already exists
      qryFillTable2.SQL.Text := 'SELECT COUNT(*) FROM ClassSelectionProcessStudent WHERE ClassName = :ClassName AND MainUserID = :UserID';
    ...
    begin
      ...
      qryFillTable2.Close;
      qryFillTable2.SQL.Text := 'INSERT INTO ClassSelectionProcessStudent (ClassName, MainUserID) VALUES (:ClassName, :MainUserID)';
      qryFillTable2.Parameters.ParamByName('ClassName').Value := ClassToAdd;
      ...
      qryFillTable2.SQL.Text := 'SELECT ClassName FROM ClassSelectionProcessStudent WHERE MainUserID = :UserID';
      qryFillTable2.Parameters.ParamByName('UserID').Value := CurrentUserID;
      qryFillTable2.Open;
...
procedure TFillInDetailsFormStudent.imgRemoveClassClick(Sender: TObject);
begin
  ClassToRemove := edtRemoveClass.Text;
   conDelete.Open;
   conDelete.BeginTrans;
  // Remove the row from the database
  qryFillTable2.SQL.Text := 'DELETE FROM ClassSelectionProcessStudent WHERE ClassName = :ClassName AND MainUserID = :UserID';
  ...
  // Refresh the dataset associated with the grid to reflect the changes
  qryFillTable2.SQL.Text := 'SELECT SelectionID, ClassName, MainUserID FROM ClassSelectionProcessStudent WHERE MainUserID = :UserID';
  qryFillTable2.Parameters.ParamByName('UserID').Value := CurrentUserID;
  qryFillTable2.Open;
...
function TFillInDetailsFormStudent.GetSubjectID(const ClassName: string): string;
begin
...
procedure TFillInDetailsFormStudent.imgFinalSubmitClick(Sender: TObject);
var
  StudentID, SubjectID: string;
  DetailFillIn: Boolean;
begin
  // Get the StudentID associated with the current user
  qryStudentID.SQL.Text := 'SELECT StudentID FROM Student WHERE MainUserID = :UserID';
  qryStudentID.Parameters.ParamByName('UserID').Value := CurrentUserID;
  qryStudentID.Open;
  if not qryStudentID.IsEmpty then
    StudentID := qryStudentID.FieldByName('StudentID').AsString
  else
  begin
    ShowMessage('Student ID not found for the current user.');
    Exit; // Exit the procedure if StudentID is not found
  end;

  // Select ClassName from ClassSelectionProcessStudent for the current user
  qrySelectClasses.SQL.Text := 'SELECT ClassName FROM ClassSelectionProcessStudent WHERE MainUserID = :UserID';
  ...
  begin
    ...
    if SubjectID <> '' then
    begin
      // Insert ClassName, StudentID, and SubjectID into qryAddClassesFinal
      qryAddClassesFinal.SQL.Text := 'INSERT INTO StudentClass (ClassName, StudentID, SubjectID) ' +
                                     'VALUES (:ClassName, :StudentID, :SubjectID)';
      qryAddClassesFinal.Parameters.ParamByName('ClassName').Value := qrySelectClasses.FieldByName('ClassName').AsString;
      qryAddClassesFinal.Parameters.ParamByName('StudentID').Value := StudentID;
      qryAddClassesFinal.Parameters.ParamByName('SubjectID').Value := SubjectID;
